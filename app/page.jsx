...
export default function Home() {
  ...
  const handleCopyUrl = async (project) => {
    const projectText = [`案件ID: ${project.id}`, `案件名: ${project.title}`].join("\n");
    try {
      await navigator.clipboard?.writeText(projectText);
      setNotice("案件情報をコピーしました");
    } catch {
      setNotice(`コピー対象: ${project.id} / ${project.title}`);
    }
    setMenuProjectId(null);
  };
  ...
  const applyKeyword = () => {
    const nextSearch = keywordDraft.include.trim();
    const nextFilterValues = { ...filterValues, exclude: keywordDraft.exclude.trim() };
  ...
  return (
    <main className="console-app">
      ...
      <SearchToolbar
        ...
        onOpenKeyword={() => {
          closeMenus();
          setKeywordDraft({ include: search, exclude: filterValues.exclude || "" });
