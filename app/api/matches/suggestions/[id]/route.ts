...
export async function GET(request: Request, context: { params: { id: string } }) {
  try {
    ...
    if (!isUuid(context.params.id)) return NextResponse.json({ message: "Invalid match suggestion id" }, { status: 400 });

    const result = await getMatchSuggestionDetail(prisma, context.params.id);
