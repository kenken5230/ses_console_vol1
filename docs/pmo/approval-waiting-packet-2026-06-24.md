...
> Update 2026-07-01: #149 (`256a443`) and #150 (`17c632b`) are now merged into
> `main` (`#148 -> #149 -> #150`), and `test:search-history-ui-context` is wired
> in `package.json`. #149 items below that read as Draft/open or pending owner
> approval are historical and resolved. Current live snapshot: `PROGRESS.md`.

## Current GitHub State

- Open PRs: none blocking this packet. #149 (SearchHistory UI context test wiring) merged at `256a443` and #150 (next-approval-gates refresh) merged at `17c632b`; both changed docs/`package.json`-level content only. #147 was this consolidated approval packet's earlier refresh branch.
...
- Recent docs-only sync/planning PRs: #138 through #146 are closed/merged; #148 is merged runtime diagnostics with secret-safe logging; #149 is merged at `256a443`, wiring `test:search-history-ui-context` into `package.json`; #150 is merged at `17c632b`.
...
| 2 | #149 SearchHistory UI context test wiring | **Merged** at `256a443`; `test:search-history-ui-context` wired into `package.json` (aggregate `npm test` included), Vercel checks passed, no DB/schema/env/lockfile/deletion impact. | Done — no further approval needed. | Completed; retained for history. |
...
| 6 | SearchHistory additional DB smoke / user isolation | DB-backed flow is merged and local normal-login QA passed. #135 added an optional local/test DB smoke packet. `test:search-history` passed on 2026-06-26; #149 (merged at `256a443`) wired the existing UI-context test into `npm test`. | Local/test-only smoke with exact users, rows, cleanup, and audit separation. | Lower priority unless SearchHistory behavior becomes suspect. |
...
2. **(Done) #149 Ready/merge**: merged at `256a443`; `test:search-history-ui-context` is wired into `npm test`. No action needed.
