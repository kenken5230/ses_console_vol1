...
if (process.env.ENFORCE_CANDIDATE_UI_FILE_SCOPE === "1") {
  for (const filePath of touchedFilesFromGit()) {
    assert(
      ...
      `project candidate display file-scope guard failed for ${filePath}. Set ENFORCE_CANDIDATE_UI_FILE_SCOPE=1 only when validating the candidate UI PR file set.`
    );
  }
}
