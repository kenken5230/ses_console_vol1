...
| Item | State | Notes |
|---|---|---|
| Snapshot source commit | `17c632b22bc438140fbf012c7305602f077baebf` | This snapshot was refreshed after PR #150 merge: `Merge pull request #150 from kenken5230/codex/next-approval-gates-refresh-20260626`. |
| Recent final PR results | #82-#103 are historical; #104 merged at `7abfaa4`; #105 at `b932ed0`; #106 at `b017e36`; #107 at `1f05e76`; #108 at `03e60e2`; #109 at `afb98ae`; #110 at `b426940`; #111 at `47b96b3`; #112 at `98b41a3`; #113 at `ca15328`; #115 at `5024bf1`; #116 at `43b08db`; #117 at `5c24052`; #118 at `c23c619`; #119 at `ffce54f`; #120 at `f9261f2`; #121 at `b140b75`; #122 at `cbbdeb8`; #123 at `a92031c`; #124 at `8419b4f`; #125 at `d227234`; #126 at `93cf98a`; #127 at `124d9f2`; #128 at `4afb596`; #129 at `10d8342`; #130 at `9506680`; #131 at `d86f2b9`; #132 at `02cd639`; #133 at `bc33a6d`; #134 at `e6c24b9`; #135 at `da053ce`; #136 at `39f3d21`; #137 at `e6d1d6b`; #138 at `4805466`; #139 at `ce0c294`; #140 at `e489508`; #141 at `0265ae0`; #142 at `23c9963`; #143 at `497cd30`; #144 at `66bb643`; #145 at `94952cb`; #146 at `e2f96b9`; #148 at `0ec3778`; #149 at `256a443`; #150 at `17c632b` | Keep only final outcomes here. Details through #87 are in `docs/status/progress-log-2026-06-20.md`; #89 merge/deploy status is in `docs/status/progress-log-2026-06-23.md`; #104-#150 current-state sync is summarized in `docs/status/post143-next-gates-2026-06-26.md`, `docs/status/dbfree-verification-refresh-2026-06-26.md`, `docs/status/auth-login-readiness-2026-06-26.md`, `docs/pmo/worktree-cleanup-permission-report-2026-06-26.md`, and `docs/pmo/next-approval-gates-2026-06-26.md`. |
...
| Auth/login readiness | A clean login recovery worktree confirmed local login cannot work without `DATABASE_URL` and `AUTH_SECRET` in the runtime process. A secret-safe readiness diagnostic is being added to check env presence, SMTP readiness, active user counts, and active users with password hashes without printing secrets or individual emails. | Run `npm.cmd run auth:login-readiness` in the affected runtime/DB context. If SMTP is missing or active users lack password hashes, proceed through an approved operational recovery gate; do not perform production/staging/shared DB writes without approval. |
| SearchHistory DB-backed work | DB-backed SearchHistory is already merged through #57; #91 restored saved filter/sort/page-size application; #102 synchronized current status docs plus chip key hardening; #107 added the Browser QA plan; #112 merged the UI context guard. Local/test normal-login SearchHistory save/list/public-response/cleanup QA passed and is recorded on #112. #135 added an optional local/test own-user-isolation DB smoke approval packet. On 2026-06-26, `test:search-history` passed again, and #149 wired `test:search-history-ui-context` into package scripts and aggregate `npm test`. | Remaining optional gates are production login-after read-only UI verification and optional local/test DB smoke with approved fixture users. Do not use auth bypass, cookie injection, token injection, or production/staging/shared DB writes. |
...
| Open PRs | The only open PR observed after #150 is old Draft PR #147, which is conflicting and superseded by #150. Open issues were 0. #149 and #150 are closed/merged. | Keep #147 on HOLD or close it only after the PR-state gate; start any new product/runtime work from latest `origin/main` in a separate clean branch/worktree. |

## Next Work Candidates

1. Complete production login-after read-only screen verification for merged UI flows using normal login only.
2. Resolve production login/password-reset readiness by deciding the Vercel production env/config gate; #148 diagnostic code is merged, but production env changes/redeploy remain owner-gated.
3. Decide whether old Draft PR #147 should be closed as superseded by #150; do not close without the PR-state gate.
4. Prepare a Person owner link DB-connected preflight only after local/test target classification and fixture approval; DB-free API/route/contract/UI tests are green.
5. Keep Gmail company apply implementation on HOLD until the converged design receives a separate implementation/DB gate approval.
6. Decide how to handle stale worktree metadata permission-denied failures using the permission report; do not run raw deletion, `--force`, worktree remove, or branch deletion without a new explicit plan.
7. Decide whether CSV/source tracking can move from HOLD to a local/test DB schema verification gate; DB-free tests are green, but do not run migration/schema/apply yet.
8. Use `docs/pmo/next-approval-gates-2026-06-26.md` as the consolidated owner-choice packet before executing any remaining gated work.
