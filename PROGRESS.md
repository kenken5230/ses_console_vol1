...
| Person owner link HTTP smoke | Runbook and read-only preflight preparation are merged in #84; #94 hardened the preflight and route smoke safety expectations. The real HTTP smoke body has not been executed. Sequence 1 DB pre-gate pack is in `docs/status/sequence1-db-pre-gate-pack-2026-06-23.md`. | Next owner decision is whether to allow a local/test-only, read-only DB connection preflight for one synthetic/disposable Person fixture. DB write smoke still requires separate approval. |
