...
| Project company/contact role link | Contract (#83), API (#85), shared safety policy (#86), progress snapshot/log split (#87), and docs sync (#88) are merged. PR #89 head has implemented the guarded project detail UI in a clean worktree. Validation/status recording is pending. Real DB write smoke has not been run. | Complete validation/status recording from the clean worktree. Run smoke only under the approved runbook/fixture process. |
...
2. Finish validation for the guarded Project company/contact role link UI on top of the merged API.
